...
async def get_camera_images(camera_id: str, user: User = Depends(require_login), db: Session = Depends(get_db)):
    camera = db.query(Camera).filter(Camera.camera_id == camera_id).first()
    ...
    for img in images:
        ...
        if url:
            image_data.append({'url': url, 'timestamp': img['timestamp'].isoformat(),
                               'size': img['size'], 'key': img['key']})
...
async def test_s3():
    ...
    try:
        ...
        try: s3_client.head_bucket(Bucket=AWS_BUCKET); bucket_exists = True
        except: bucket_exists = False
        ...
        return {'bucket': AWS_BUCKET, 'bucket_exists': bucket_exists,
                'objects_found': len(objects), 'objects': objects, 'credentials_valid': True}
    except Exception as e:
        return {'error': str(e)}
